...
# Set up directory
setwd("~/rao_lab/Single Cell/single_cell/HNSCC")
...
# PREPROCESSING
# load in data
HNSCC_371_neg.data <- Read10X("data_primary_tumors/Run_2961/Sample_HN371doublenegative/filtered_feature_bc_matrix/")
HNSCC_371_pos.data <- Read10X("data_primary_tumors/Run_2961/Sample_HN371doublepositive/filtered_feature_bc_matrix/")
HNSCC_372_neg.data <- Read10X("data_primary_tumors/Run_2996/Sample_HN372_ALDHnegCD44neg/filtered_feature_bc_matrix/")
HNSCC_372_pos.data <- Read10X("data_primary_tumors/Run_2996/Sample_HN372_ALDHposCD44pos/filtered_feature_bc_matrix/")
HNSCC_373_neg.data <- Read10X("data_primary_tumors/Run_3048/Sample_HN373_ALDHnegCD44neg/filtered_feature_bc_matrix/")
HNSCC_373_pos.data <- Read10X("data_primary_tumors/Run_3048/Sample_HN373_ALDHposCD44pos/filtered_feature_bc_matrix/")
HNSCC_375_neg.data <- Read10X("data_primary_tumors/Run_3078/Sample_HN375neg/filtered_feature_bc_matrix/")
HNSCC_375_pos.data <- Read10X("data_primary_tumors/Run_3078/Sample_HN375pos/filtered_feature_bc_matrix/")
HNSCC_378_neg.data <- Read10X("data_primary_tumors/NovaA-138/Sample_HN378_ALDHnegCD44neg/filtered_feature_bc_matrix/")
HNSCC_378_pos.data <- Read10X("data_primary_tumors/NovaA-138/Sample_HN378_ALDHposCD44pos/filtered_feature_bc_matrix/")

# create Seurat obejcts
HNSCC_371_neg <- CreateSeuratObject(counts = HNSCC_371_neg.data, project = 'HNSCC_371_neg', min.cells = 3, min.features = 100)
HNSCC_371_pos <- CreateSeuratObject(counts = HNSCC_371_pos.data, project = 'HNSCC_371_pos', min.cells = 3, min.features = 100)
HNSCC_372_neg <- CreateSeuratObject(counts = HNSCC_372_neg.data, project = 'HNSCC_372_neg', min.cells = 3, min.features = 100)
HNSCC_372_pos <- CreateSeuratObject(counts = HNSCC_372_pos.data, project = 'HNSCC_372_pos', min.cells = 3, min.features = 100)
HNSCC_373_neg <- CreateSeuratObject(counts = HNSCC_373_neg.data, project = 'HNSCC_373_neg', min.cells = 3, min.features = 100)
HNSCC_373_pos <- CreateSeuratObject(counts = HNSCC_373_pos.data, project = 'HNSCC_373_pos', min.cells = 3, min.features = 100)
HNSCC_375_neg <- CreateSeuratObject(counts = HNSCC_375_neg.data, project = 'HNSCC_375_neg', min.cells = 3, min.features = 100)
HNSCC_375_pos <- CreateSeuratObject(counts = HNSCC_375_pos.data, project = 'HNSCC_375_pos', min.cells = 3, min.features = 100)
HNSCC_378_neg <- CreateSeuratObject(counts = HNSCC_378_neg.data, project = 'HNSCC_378_neg', min.cells = 3, min.features = 100)
HNSCC_378_pos <- CreateSeuratObject(counts = HNSCC_378_pos.data, project = 'HNSCC_378_pos', min.cells = 3, min.features = 100)
...
save(HNSCC_merge, file = 'HNSCC_merge_primary_tumors_raw_020620.RData')
...
# Filter cells
HNSCC_merge <- subset(x = HNSCC_merge, subset = nCount_RNA < 50000 & nFeature_RNA < 6250 & percent.mt < 25)
...
old_HNSCC <- HNSCC_merge # HNSCC_env.RDataTmp.RData
new_HNSCC <- HNSCC_merge # HNSCC_merge_primary_tumors_filtered_020620.RData

old_umap <- DimPlot(old_HNSCC, reduction = 'umap')
new_umap <- DimPlot(new_HNSCC, reduction = 'umap')
CombinePlots(plots = list(old_umap, new_umap))

old_aldh_genes <- rownames(old_HNSCC)[grep('^ALDH', rownames(old_HNSCC))]
old_var_aldh_genes <- VariableFeatures(old_HNSCC)[grep('^ALDH', VariableFeatures(old_HNSCC))]
VlnPlot(old_HNSCC, features = old_var_aldh_genes)
VlnPlot(old_HNSCC, features = old_aldh_genes[1:6])
VlnPlot(old_HNSCC, features = old_aldh_genes[7:12])
VlnPlot(old_HNSCC, features = old_aldh_genes[13:19])

new_aldh_genes <- rownames(new_HNSCC)[grep('^ALDH', rownames(new_HNSCC))]
new_var_aldh_genes <- VariableFeatures(new_HNSCC)[grep('^ALDH', VariableFeatures(new_HNSCC))]
VlnPlot(new_HNSCC, features = new_var_aldh_genes)
VlnPlot(new_HNSCC, features = new_aldh_genes[1:6])
VlnPlot(new_HNSCC, features = new_aldh_genes[7:12])
VlnPlot(new_HNSCC, features = new_aldh_genes[13:19])

'------------------------------------------------------------------'
# Look for similar genes expressed in primary tumors and cell lines
primary_DE <- read.csv('HNSCC_DE_all_clusters_batch_corrected_103119.csv', header = T, stringsAsFactors = F)
cell_line_DE <- read.csv('HNSCC_cell_line_DE_state_batch_012020.csv', header = T, stringsAsFactors = F)
